...
| **ConsentContext** (FD-10) | `sgp.py` diligence gate (buyer-local, not exchanged) | — (`consent` mentioned 44×, no type) | 67 buyer / 44 seller mentions of consent with **no exchanged type** (plan §4.3) | EP-1.2 placeholder, **built out in EP-10.4** (see below): `applicable_regimes`, `gpp_string`/`gpp_section_ids` (GPP), `tcf_string` (TCF), `DiligenceStatus` + `verified_at` (SGP / IAB Diligence Platform). Slots exist on `Quote`, `Deal`, and `Order`. |

## EP-10 primitive-gap backfill

Additive changes only; every new Deal/Order/Quote field is optional so
EP-1.2 fixtures still validate. Acronyms defined once: GPP = Global
Privacy Platform; TCF = Transparency & Consent Framework; SGP = SafeGuard
Privacy / IAB Diligence Platform; schain = OpenRTB supply chain; asi =
advertising system identifier; sid = seller id; hp = handled-payment flag;
GRP = gross rating point.

| Primitive | Status before | What EP-10 did | Key fields |
|---|---|---|---|
| **DecisionRecord** (10.1) | ABSENT (only named in `events/__init__.py` as a future backfill) | NEW persisted domain object in `primitives/audit.py` — the durable record of WHY a money/state decision was made. DISTINCT from the transient `events.Event` bus message (a booking emits BOTH). References its subject by id; never embedded on Deal/Order/Quote. | `decision_id`, `subject_type`/`subject_id`, `decision_type` (booking/negotiation_concession/approval/…), `actor` (`DecisionActor`: agent id + human-or-machine `kind`), `inputs` (`DecisionInputRef[]` — refs/hashes of counterparty inputs + model outputs, **never raw prompts**), `rationale` (structured `DecisionRationale`: summary + factors + policy_refs), `money_effect: Money?`, tz-aware `occurred_at`, `correlation_id`/`session_id`/`negotiation_id`. |
| **Makegood** (10.2) | PARTIAL — EP-1.2 shipped `ChangeType.MAKEGOOD` + `MakegoodDetails` (shortfall_grps, original_daypart, target_demo, preferred_dayparts, notes) | Completed the compensation terms on the existing typed subtype (per FD-6 makegood is NOT a standalone primitive). No parallel type created. | Added `owed_impressions`, `replacement_flight_start`/`replacement_flight_end`, and `status` (`MakegoodStatus`: proposed/accepted/scheduled/delivered/rejected). Human reason stays on `ChangeRequest.reason` (not duplicated). |
| **SupplyChain** (10.3) | ABSENT (`standards.py` recorded schain as "not modeled") | NEW `primitives/supply_chain.py`: `SupplyChain` (`complete` 0/1, `ver`, `nodes[]`) + `SupplyChainNode` using OpenRTB field names verbatim (`asi`, `sid`, `hp`, `rid`, `name`, `domain`, `ext`). Carried optionally on `Deal.supply_chain`. | Plus `SellersJsonEntry` (`seller_id`, `name`, `domain`, `seller_type` PUBLISHER/INTERMEDIARY/BOTH, `is_confidential`, `comment`) mirroring an IAB Tech Lab sellers.json entry. Standards registry updated: modeled + round-trip/schema checked, still UNVERIFIED against the AdCOM/OpenRTB and sellers.json publications. |
| **ConsentContext** (10.4) | PARTIAL — EP-1.2 minimal placeholder | Built out on the SAME type (field names preserved for backward compat; purely additive). | Added `gdpr_applies: bool?` (gates the TCF string) and `us_privacy: str?` (US Privacy / CCPA string). Existing `gpp_string`/`gpp_section_ids`/`tcf_string`/`diligence_status`/`verified_at` unchanged. |
| **CreativeApproval** (10.5b) | Creative already carried `review_status` (live state) but no approval **record** | NEW `CreativeApproval` in `creative.py`: the first-class, timestamped record of one seller review decision, complementary to `Creative.review_status`. Reuses `ReviewStatus` so the vocabulary never forks. | `approval_id`, `creative_id`, `status` (ReviewStatus), `reviewer`, `reason?`, tz-aware `occurred_at`. |
| **Avails** (10.5a) | Correctly ABSENT from `primitives/` | Confirmed: Avails is a query, not a persisted primitive (G9 / plan §4.4). No primitive-like Avails exists; it belongs in `protocol/` if/when a route is added. No change. | — |
| **RateCard** (10.5c) | DONE (FD-9 per-pair private rates) | Verified correct; no change. | — |
