...
| **Avails** | `AvailsRequest`/`AvailsResponse` in `models/opendirect.py`: spec-lowercase `productid`/`startdate`/`enddate` + camelCase extension fields; NO `ge=0` on `requestedImpressions`, NO end-after-start validation | `POST /products/avails` (shipped v2.1.0, public docs): same field set, `requestedImpressions >= 0`, `enddate > startdate` enforced, honest-availability derivation; pads absent optionals with explicit `null` (`deliveryConfidence`/`guaranteedImpressions`/`availableTargeting`) | `protocol.AvailsRequest`/`protocol.AvailsResponse`: the served dialect wins byte-for-byte (surface is live and publicly documented). Seller's stricter validation wins (`ge=0`, end-after-start with naive/aware normalization). Policy: `availableImpressions` REQUIRED (uncapped products report requested-as-available); `deliveryConfidence` OPTIONAL and OMITTED entirely when there is no forecast data source (never fabricated, never null-padded; readers tolerate legacy `null`); `guaranteedImpressions` present ONLY for PG-capable products. Money fields stay FLOAT — a documented FD-11 exception to preserve the shipped wire; `Money` migration is next-major. | Buyer gains request validation it previously lacked. Conformant emitters omit valueless optionals instead of null-padding (seller <= v2.1.0 null-padding remains parseable). |
| **Avails — OpenDirect 2.1 spec dialect (v0.5.0 convergence)** | Legacy simplified profile only (single-product `productid`, camelCase extension fields) — Off-Spec against the published `ProductAvailsSearch`/`Avails` tables (missing `accountid`/`advertiserbrandid`, scalar vs array, non-spec extras, no collection envelope, no `availsstatus`) | Same simplified profile; a strictly spec-shaped request was REJECTED (`productid` required) | The published normative tables now live in the contract alongside the legacy profile: `ProductAvailsSearch` (multi-product `productids` + required `accountid`/`advertiserbrandid`), `Avails` (+ `availsstatus`: `AvailsStatus`/`ProductTargeting`, spec status + reason enums), and the `AvailsCollection` `{"avails": [...]}` envelope per the spec's Collection Objects table. Servers accept BOTH dialects (`parse_avails_request` discriminates on `productids` vs `productid`); the RESPONSE DIALECT FOLLOWS THE REQUEST DIALECT. Legacy extension fields travel in spec slots on the spec dialect: `requestedImpressions`/`budget` as minted Investment `producttargeting` entries (`datasource=iab-agentic-primitives`, `target=requestedimpressions|budget`), the legacy targeting dict as the AdCOM Segment `targeting` array. `avails_from_simplified` derives `availsstatus` from the honest-availability numbers (full → `Available`; capped → `Partially Available`/`Booked`; zero → `Unavailable`/`Booked`). Spec ambiguities recorded, not resolved in our favor: `availability`'s reference to an undefined request `quantity`; the spec examples' PascalCase and `availsstatus`-as-array vs the normative tables. | NONE on existing wire traffic: legacy round-trips are pinned unchanged and legacy requests get legacy responses. Additive: a spec-shaped request now succeeds instead of 422ing. Emitting the spec dialect requires `accountid`/`advertiserbrandid` the legacy profile never carried — buyers without account context stay on the legacy dialect. |
